...
extern int Lidar_forward_stop_flag;
    ...
    for (int p=0; p<90; p++) { // for all 90 packets
        ...
        for (int i=0; i<4; i++) { // process 4 chunks per packet
            ...
            if (!invalid_data_flag(data)) {
                ...
                if (((angle_degrees < 35) || (angle_degrees >325)) && (dist_mm(data)< 350))
                {
                    //printf("NEW STOP!!!!!!!!!\n");
                    Lidar_forward_stop_flag = 1;
                }
                else if (((angle_degrees < 35) || (angle_degrees >325)) && (dist_mm(data)> 400))
                {
                    //printf("                  \n");
                    Lidar_forward_stop_flag = 0;
                }
